...
train_file = os.path.join(root, "..","data", "processed", "ml_datasets", f"{dataset}.csv")
model_folder = os.path.join(root, "..", "models", f"{dataset}")
...
model.save(f"{model_folder}.zip", onnx=True)

with open(os.path.join(root, "..","models", f"report_{dataset}.json"), "w") as f:
